...
この試験では、次の技術タスクを達成する能力を測定します : SAP ワークロードを Azure に移行する。SAP ワークロードをサポートするインフラストラクチャを設計、実装する。高可用性とディザスター リカバリーを設計、実装する。Azure で SAP ワークロードをメンテナンスする。  
...
この試験の受験者は、SAP アプリケーションとデータベースのオペレーティング システム、Azure 管理、Infrastructure as Code、クラウド インフラストラクチャ、高可用性、ディザスター リカバリー、バックアップ、セキュリティ、およびデータ保護に精通している必要があります。  
(As an exam candidate, you should be familiar with operating systems for SAP applications and databases, Azure administration, infrastructure as code, cloud infrastructure, high availability, disaster recovery, backup, security, and data protection.)
...
(Design and implement governance and compliance by using Azure Policy)
  - SAP ワークロードに対する Azure Active Directory (Azure AD)、Azure Active Directory Domain Services (Azure AD DS)、Active Directory 認証を設計、統合する  
(Design and integrate Azure Active Directory (Azure AD), Azure Active Directory Domain Services (Azure AD DS), and Active Directory authentication for SAP workloads)
...
(Design for service-level agreement (SLA) considerations)
  - SAP ワークロード 用に可用性セットや可用性ゾーンを設計、デプロイする  
(Design and deploy availability sets and availability zones for SAP workloads)
