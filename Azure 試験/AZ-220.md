...
## 試験について (意訳)
**※2022/07/28 に英語版が改定 (アップデート) されました。**
- Exam AZ-220: Microsoft AzureIoTDeveloper–Skills Measured  
https://query.prod.cms.rt.microsoft.com/cms/api/am/binary/RE4nBeC

この試験では、Azure IoT Hub ソリューション インフラストラクチャをセットアップする能力を測定します : デバイスのプロビジョニングと管理。IoT Edge の実装。また、IoT ソリューションのトラブルシューティングだけでなく、ビジネス統合とデータ処理に関する知識もテストされます。  
(This exam measures your ability to set up the Azure IoT Hub solution　infrastructure; provision and　manage devices; and implement IoT Edge. You will also be tested on your knowledge of business　integration and processing data, as well as troubleshooting IoT solutions.)

この試験の受験者は、IoT ソリューションのクラウドとデバイスの部分を作成および維持するための専門知識を持っている必要があります。クラウド サービスを用いてデバイスを構成および維持することに加え、物理デバイスをセットアップできることも要求されます。    
(As an exam candidate, you should have subject matter expertise in creating and maintaining the cloud　and device portions of an IoT solution. In addition to configuring and maintaining the devices by using　cloud services, you are also expected to be able to set up the physical devices.)
...
### 機能グループ
#### Azure IoT Hub ソリューション インフラストラクチャのセットアップ (Set up the Azure IoT Hub solution infrastructure) (10-15%)
- IoT Hub を作成、構成する  
...
#### IoT Edge の実装 (Implement IoT Edge) (15-20%)
...
#### データの処理と管理 (Process and manage data) (15-20%)
...
(Create a Time Series Insights environmen)
...
#### IoT ソリューションの監視、トラブルシュート、および最適化 (Monitor, troubleshoot, and optimize IoT solutions) (15-20%)
- 正常性監視を構成する  
...
#### セキュリティの実装 (Implement security) (15-20%)
