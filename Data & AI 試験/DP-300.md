...
この試験の受験者は、Azure SQL Database Service で構築された複数のワークロードをサポートするように設計されたデータベース ソリューションの構築に関する専門知識を持っている必要があります。この試験の受験者は、SQL Server や SQL Database Service で構築されたオンプレミスおよびクラウド データベースを管理するデータベース管理者です。
(Candidates for this exam should have subject matter expertise in building database solutions that are designed to support multiple workloads built with Azure SQL database services. Candidates for this exam are database administrators who manage on-premises and cloud databases built with SQL Server and SQL database services.)

Azure database administrator は、SQL Server や SQL データベース サービス上に構築されたクラウド ネイティブおよびハイブリッド データ プラットフォーム ソリューションの運用面を実装および管理します。この専門ロールでは、管理目的での T-SQL の使用に関する知識の適用を含むさまざまな方法、およびツールを用いて日常業務を実行および自動化します。
...
また、データベース ソリューションの管理、可用性、セキュリティ、およびパフォーマンスの監視と最適化を担当します。データベースを Azure に移行するための移行戦略を評価および実装します。さらに、Azure データ エンジニア、Azure ソリューション アーキテクト、Azure 開発者、およびその他の専門家と協力して、データ プラットフォーム ソリューションの運用面を管理します。
