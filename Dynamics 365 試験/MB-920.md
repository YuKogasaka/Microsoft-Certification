# MB-920 : Microsoft Dynamics 365 Fundamentals Finance and Operations Apps (ERP)
Microsoft 認定資格「Microsoft Certified : Dynamics 365 Fundamentals Finance and Operations Apps (ERP)」の対象試験です。
- Exam MB-920: Microsoft Dynamics 365 Fundamentals Finance and Operations Apps (ERP) - Learn | Microsoft Docs  
https://docs.microsoft.com/learn/certifications/exams/MB-920

## 評価されるスキルについて (意訳)
**※2022/12/20 に英語版が改定 (アップデート) されました。**
- Exam MB-920: Microsoft Dynamics 365 Fundamentals Finance and Operations Apps (ERP)–Skills Measured  
https://query.prod.cms.rt.microsoft.com/cms/api/am/binary/RE4JFBT

この試験では、Dynamics 365 の財務および運用アプリの特徴と機能をカバーしています。
(This exam covers the features and capabilities of Dynamics 365 finance and operations apps.)

対象者は、情報技術 (IT) 環境に関する一般的な知識、もしくは関連する実務経験を持っている必要があります。 また、財務原則と事業運営の基本的な理解も必要となります。
(As an exam candidate, you should have general knowledge of or relevant working experience with an Information Technology (IT) environment. You should also have a fundamental understanding of financial principles and business operations.)

### 対象となる領域 : 試験で評価されるスキル
試験は定期的にアップデートされ、役割を果たすために必要なスキルが反映されます。試験を受ける時期に応じて、スキル測定目標の 2 つのバージョンが含まれています。  
(Our exams are updated periodically to reflect skills that are required to perform a role. We have included two versions of the Skills Measured objectives depending on when you are taking the exam.)

常に、試験の英語版を最初にアップデートします。一部の試験は他の言語にローカライズされており、英語版がアップデートされてから約 8 週間後にアップデートされます。その他の利用可能な言語は、**試験の詳細** Web ページの**試験のスケジュール**セクションに一覧表示されています。ご希望の言語で試験を利用できない場合は、試験を完了するために 30 分の追加時間をリクエスト可可能です。  
(We always update the English language version of the exam first. Some exams are localized into other languages, and those are updated approximately eight weeks after the English version is updated. Other available languages are listed in the Schedule Exam section of the Exam Details webpage. If the exam isn't available in your preferred language, you can request an additional 30 minutes to complete the exam.)

注 : 評価される各スキル続いて記載している箇条書きの一覧は、スキルの評価項目 (どのように評価しているかを示す) となります。関連するトピックが試験内でカバーされている亜愛があります。
(Note : The bullets that follow each of the skills measured are intended to illustrate how we are assessing that skill. Related topics may be covered in the exam.)

注 : ほとんどの問題は、GA (General Availability) となった機能に関するものです。プレビュー機能が一般的に私用されている場合、それらに関する問題が含まれる場合があります。
(Note : Most questions cover features that are general availability (GA). The exam may contain questions on Preview features if those features are commonly used.)

### 機能グループ
#### Dynamics 365 Supply Chain Management を説明する (Describe Dynamics 365 Supply Chain Management) (25–30%)
- コア機能を識別する  
  (Identify core capabilities)
  - ユースケースと Supply Chain Management 機能について説明する  
    (Describe use cases and Supply Chain Management capabilities)
  - 利用可能な Supply Chain Management モジュールについて説明する  
    (Describe available Supply Chain Management modules)
  - 他のアプリやテクノロジと Supply Chain Management との統合オプションについて説明する  
    (Describe options for integrating Supply Chain Management with other apps and technologies)
- 製品管理機能について説明する  
  (Describe product management capabilities)
  - 製品と製品のリリースについて説明する  
    (Describe products and release products)
  - 製品マスターについて説明する  
    (Describe product masters)
  - 製品バリアントについて説明する  
    (Describe product variants)
- 購買力と販売力について説明する  
  (Describe purchase and sales capabilities)
  - 仕入先と発注書を作成するプロセスについて説明する  
    (Describe the process for creating vendors and purchase orders)
  - 顧客と販売注文を作成するプロセスについて説明する  
    (Describe the process for creating customers and sales orders)
  - 貿易協定と契約について説明する  
    (Describe trade agreements and contracts)
  - ピッキング、梱包、および在庫の確保について説明する  
    (Describe picking, packing, and reserving stock)
- 製造戦略の説明  
  (Describe manufacturing strategies)
  - リーン、ディスクリート、およびプロセス製造について説明する  
    (Describe lean, discrete, and process manufacturing)
  - 製造オーダーに関連する部品表 (BOM)、ルート、および工程を説明する  
    (Describe bills of materials (BOMs), routes, and operations as they relate to production orders)
  - マスター プランニングについて説明する
    (Describe master planning)
- インベントリとウェアハウジング機能について説明する  
  (Describe inventory and warehousing capabilities)
  - インベントリ ジャーナルについて説明する  
    (Describe inventory journals)
  - 在庫レベルについて説明する  
    (Describe stock levels)
  - 手持在庫機能について説明する
    (Describe on-hand inventory capabilities)
  - 場所を含む倉庫コンポーネントについて説明する  
    (Describe warehouse components including locations)
#### Dynamics 365 Finance を説明する (Describe Dynamics 365 Finance) (25–30%)
- Dynamics 365 Finance のコア機能について説明する  
  (Describe Dynamics 365 Finance core capabilities)
  - 法人のユースケースについて説明する  
    (Describe use cases for legal entities)
  - Dynamics 365 Finance がその地域の税レポート要件を処理する方法について説明する  
    (Describe how Dynamics 365 Finance handles regional tax reporting requirements)
  - 会計の概念について説明する  
    (Describe accounting concepts)
  - ジャーナリングの概念について説明する  
    (Describe journaling concepts)
- 一般会計を説明する  
  (Describe general ledgers)
  - 主勘定、貸借対照表、損益計算書を含む勘定科目表について説明する  
    (Describe charts of accounts including main accounts, balance sheets, and income statements)
  - 財務分析コードと分析コード セットの概念について説明する  
    (Describe financial dimensions and dimension set concepts)
  - 定期的な財務プロセスについて説明する  
    (Describe periodic financial processes)
- 買掛金と売掛金について説明する
  (Describe accounts payables and accounts receivables)
  - 仕入先、発注書、および仕入先請求書を含む主要な買掛金コンポーネントについて説明する  
    (Describe core accounts payable components including vendors, purchase orders, and vendor invoices)
  - スリーウェイ マッチングの概念を含むベンダーの支払いと決済について説明する  
    (Describe vendor payments and settlements including three-way matching concepts)
  - 顧客、顧客の請求書、自由書式の請求書など、主要な売掛金コンポーネントについて説明する  
    (Describe core accounts receivable components including customers, customer invoices, and free text invoices)
  - 与信および回収プロセスについて説明する  
    (Describe credit and collection processes)
- 経費管理、固定資産管理、および予算編成について説明する  
    (Describe expense management, fixed asset management, and budgeting)
  - 現金および銀行管理の概念について説明する  
    (Describe cash and bank management concepts)
  - 経費管理機能について説明する  
    (Describe expense management capabilities)
  - 固定資産管理機能について説明する  
    (Describe fixed asset management capabilities)
  - 予算作成機能について説明する  
    (Describe budgeting capabilities)
#### Dynamics 365 Commerce を説明する (Describe Dynamics 365 Commerce) (5–10%)
- Dynamics 365 Commerce コア機能を説明する  
  (Describe Dynamics 365 Commerce core capabilities)
  - 小売、オンライン、コール センター チャネルを含む、小売チャネルについて説明する  
    (Describe retail channels including retail, online, and call center channels)
  - チャネルとチャネル管理の概念について説明する  
    (Describe channels and channel management concepts)
  - 配分済み注文の管理 (DOM) を説明する  
    (Describe Distributed Order Management (DOM))
  - 製品の品揃えについて説明する  
    (Describe product assortments)
- 製品管理機能について説明する  
  (Describe product management capabilities)
  - 顧客ロイヤルティの概念について説明する  
    (Describe customer loyalty concepts)
  - Dynamics 365 Commerce がパーソナライズされたカスタマー エクスペリエンスの作成と製品の推奨の提供にどのように役立つかについて説明する  
    (Describe how Dynamics 365 Commerce helps create personalized customer experiences and provide product recommendations)
  - Dynamics 365 Commerce が Dynamics 365 Customer Insights を使用する方法について説明する  
    (Describe how Dynamics 365 Commerce uses Dynamics 365 Customer Insights)
  - Dynamics 365 Fraud Protection がシステム内の不正行為をどのように減らすかを説明する  
    (Describe how Dynamics 365 Fraud Protection reduces fraudulent activities in the system)
#### Dynamics 365 Human Resources を説明する (Describe Dynamics 365 Human Resources) (10–15%)
- コア機能を説明する  
  (Describe core capabilities)
  - 従業員と役職について説明する  
    (Describe workers and positions)
  - 報酬と福利厚生の機能について説明する  
    (Describe compensation and benefits capabilities)
  - コンプライアンス機能について説明する  
    (Describe compliance features)
  - パフォーマンス管理機能について説明する  
    (Describe performance management capabilities)
- 人事管理能力について説明する  
  (Describe personnel management capabilities)
  - セルフサービス機能について説明する  
    (Describe self-service capabilities)
  - 休暇と欠勤の機能について説明する  
    (Describe leave and absence capabilities)
  - 従業員のトレーニングと認定機能について説明する  
    (Describe employee training and certification features)
  - Dynamics 365 Human Resources や LinkedIn 間の統合オプションを特定する  
    (Identify integration options between Dynamics 365 Human Resources and LinkedIn)
#### Project Operations を説明する (Describe Project Operations) (5–10%)
- プロジェクト運営能力を識別する
  (Identify Project Operations capabilities)
  - プロジェクトベースのサービスとプロジェクトの種類について説明する  
    (Describe project-based services and project types)
  - プロジェクトの販売能力について説明する
    (Describe project sales capabilities)
  - プロジェクト契約の機能について説明する  
    (Describe project contract capabilities)
  - プロジェクトの価格設定機能について説明する  
    (Describe project pricing capabilities)
  - プロジェクトの請求と収益認識を含むプロジェクトのライフサイクルを説明する  
    (Describe the project lifecycle including project invoicing and revenue recognition)
- プロジェクトの計画と実行能力について説明する  
  (Describe project planning and execution capabilities)
  - プロジェクトの計画、実行、および分析機能について説明する  
    (Describe project planning, execution, and analysis capabilities)
  - 作業配分構造を説明する
    (Describe work breakdown structures)
  - 契約、段階、チームの割り当て、および固定価格と時間、材料の見積もりと保持契約を含むプロジェクトの特徴を特定する  
    (Identify project features including contracts, stages, team assignments, and fixed price versus time and material estimates versus retainer contracts)
  - 割り当て方法、リソースのスケジューリング、スキルを含むプロジェクト チームの管理機能について説明する  
    (Describe project team management capabilities including allocation methods, resource scheduling, and skills)
  - タスク、サブタスク、割り当て、プロジェクト計画管理などのプロジェクト スケジュール管理機能について説明する  
    (Describe project schedule management capabilities including tasks, subtasks, assignments, and project plan management)
  - 時間と費用のコストを含むプロジェクト コスト追跡機能について説明する  
    (Describe project cost tracking capabilities including time and expense costs)
#### 共有機能を説明する (Describe shared features) (10–15%)
  - レポーティング機能について説明する  
    (Describe reporting capabilities)
  - ワークスペースを含む組み込みのレポート機能について説明する  
    (Describe built-in reporting capabilities including workspaces)
  - Microsoft Excel 内から財務データを操作するためのオプションについて説明する  
    (Describe options for working with Finance data from within Microsoft Excel)
  - Power BI を用いてデータを分析するためのオプションについて説明する
    (Describe options for analyzing data by using Power BI)
  - 統合オプションについて説明する  
    (Describe integration options)
  - Microsoft Teams の統合機能について説明する  
    (Describe Microsoft Teams integration capabilities)
  - Microsoft Excel や Microsoft Word との統合の使用例について説明する  
    (Describe use cases for integrating with Microsoft Excel and Microsoft Word)
  - SharePoint Online を用いてドキュメントを管理するためのオプションについて説明する  
    (Describe options for managing documents by using SharePoint Online)
  - 電子メール統合機能を説明する  
    (Describe email integration capabilities)
  - Microsoft Dataverse 統合機能を説明する  
    (Describe Microsoft Dataverse integration capabilities)

## 関連するラーニング パス (Microsoft Learn)
- Dynamics 365 財務と運用アプリの主要機能を確認する
https://learn.microsoft.com/ja-jp/training/paths/explore-dynamics-365-finance-operations-apps/ - Learn | Microsoft Docs  
- Dynamics 365 Finance の基礎を学ぶ
https://learn.microsoft.com/ja-jp/training/paths/explore-microsoft-dynamics-365-finance/ - Learn | Microsoft Docs  
- Dynamics 365 Supply Chain Management の基礎を学ぶ
https://learn.microsoft.com/ja-jp/training/paths/explore-microsoft-dynamics-365-supply-chain-management/ - Learn | Microsoft Docs  
- Dynamics 365 Commerce の基礎を学ぶ
https://learn.microsoft.com/ja-jp/training/paths/learn-fundamentals-microsoft-dynamics-365-commerce/ - Learn | Microsoft Docs  
- Dynamics 365 Human Resources の基礎を学ぶ
https://learn.microsoft.com/ja-jp/training/paths/learn-fundamentals-microsoft-dynamics-365-human-resources/ - Learn | Microsoft Docs  
- Dynamics 365 Project Operations の基礎を学ぶ
https://learn.microsoft.com/ja-jp/training/paths/explore-microsoft-dynamics-365-project-operations-erp/ - Learn | Microsoft Docs  

## サンプル問題
- MB-920: Microsoft Dynamics 365 の基礎 (ERP) のサンプルの質問 | Microsoft Learn  
https://learn.microsoft.com/certifications/resources/mb-920-sample-questions

## 関連サイト
- Microsoft Certified: Dynamics 365 Fundamentals Finance and Operations Apps (ERP) - Learn | Microsoft Docs  
https://docs.microsoft.com/learn/certifications/d365-fundamentals-finance-and-operations-apps-erp