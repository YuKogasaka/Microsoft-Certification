...
最後に、すべての Microsoft 365 ワークロードと Azure Active Directory の機能経験があり、これらの少なくとも 1 つを管理しています。また、ネットワーク、サーバー管理、DNS、および PowerShell に関する実践的な知識が必要です。
...
(Monitor Microsoft 365 tenant health)
  ...
  (Workplace Analytics や生産性スコアを含む使用量メトリックをスケジュール、レビューする)
...
(Plan identity synchronization)
  ...
  (evaluate whether objects should be synchronized, not synchronized, or created as cloudonly)
