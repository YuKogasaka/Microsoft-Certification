...
この試験では、次の技術タスクを達成する能力を測定します : 組織の設定とリソースを管理する。メール転送アーキテクチャを計画、管理する。ハイブリッド構成と移行を計画、実装する。メッセージング環境を保護する。
...
この試験の受験者は、ハイブリッドおよびクラウド エンタープライズ環境で受信者、コンプライアンス、脅威保護、およびメール フローを展開、構成、管理、トラブルシューティング、監視するメッセージング管理者です。
...
また、認証の種類、ライセンス、および Microsoft 365 アプリケーションとの統合に関する実用的な知識が必要となります。
(As an exam candidate, you should have a working knowledge of authentication types, licensing, and integration with Microsoft 365 applications.)
...
(Create and configure connectors to secure mail flow between organizations)
  - Microsoft Defender for Office 365 でコネクターの拡張フィルター処理を構成する  
(Configure enhanced filtering for connectors in Microsoft Defender for Office 365)
