...
責任には、脅威の管理、監視、および環境全体でさまざまなセキュリティソリューションを使用することによる対応が含まれます。この役割は主に、Microsoft Sentinel、Microsoft Defender for Cloud、Microsoft 365 Defender、およびサードパーティのセキュリティ製品を用いて、脅威を調査、対応、および探索します。Security Operations Analyst は、これらのツールの運用出力を消費するため、これらのテクノロジの構成と展開における重要な利害関係者でもあります。
...
- Microsoft Defender for Office 365 を用いて本番環境に対する脅威を検出、調査、対応、修正を行う
  - Microsoft Teams、SharePoint、および OneDrive に対する脅威を検出、調査、対応、修正を行う
...
- アプリケーションの脅威を検出、調査、対応、修正を行う
  - Microsoft Defender for Cloud Apps を用いてセキュリティ リスクを特定、調査、修正を行う
  - 脅威を検出するためのアラートとレポートを生成するように Microsoft Defender for Cloud Apps を構成する
- Microsoft 365 Defender Portal でクロス ドメイン調査を管理する
  - Microsoft 365 Defender 製品全体のインシデントを管理する
  - 製品全体で承認待ちのアクションを管理する
  - 高度な脅威の探索を実行する
#### Microsoft Defender for Cloud を用いた脅威の軽減 (25-30%)
- Microsoft Defender for Cloud の実装を設計、構成する
  - 選択したターゲット サブスクリプションやワークスペースを含む Microsoft Defender for Cloud 設定を計画、構成する
  - Microsoft Defender for Cloud ロールを構成する
  - データ保持ポリシーを構成する
  - クラウド ワークロードの保護を評価、推奨する
- Microsoft Defender for Cloud でデータ ソースを取り込むためのデータ コネクタの使用を計画、実装する
  - Microsoft Defender for Cloud に取り込まれるデータ ソースを特定する
  - Azure リソースの Automated Onboarding を構成する
  - オンプレミス コンピューターを接続する
  - AWS クラウド リソースに接続する
  - GCP クラウド リソースに接続する
  - データ コレクションを構成する
- Microsoft Defender for Cloud アラート ルールを管理する
  - アラート構成を検証する
  - E メール通知をセットアップする
  - アラート抑制ルールを作成、管理する
- 自動化と修復を構成する
  - Microsoft Defender for Cloud の自動応答を構成する
  - Microsoft Defender for Cloud のワークフローの自動化を設計、構成する
  - Microsoft Defender for Cloud の推奨事項を用いてインシデントを修正する
  - Azure Resource Manager テンプレートを用いて自動応答を作成する
- Microsoft Defender for Cloud アラートとインシデントを調査する
  - Azure ワークロードに置けるアラートの種類を説明する
  - セキュリティ アラートを管理する
  - セキュリティ インシデントを管理する
  - Microsoft Defender for Cloud の脅威インテリジェンスを分析する
  - Microsoft Defender for Cloud for Key Vault アラートに応答する
  - 調査中に発見したユーザー データを管理する
#### Microsoft Sentinel を用いて脅威を軽減する (40-45%)
- Microsoft Sentinel ワークスペースを設計、構成する
  - Microsoft Sentinel ワークスペースを計画する
  - Microsoft Sentinel ロールを構成する
  - Microsoft Sentinel データ ストレージを設計する
  - Microsoft Sentinel に対するセキュリティ設定やaccessを構成する
- Microsoft Sentine でデータ ソースを取り込むためのデータ コネクタの使用を計画、実装する
  - Microsoft Sentinel に取り込むデータ ソースを特定する
  - データ コネクタの前提条件を特定する
  - Microsoft Sentinel データ コネクタを構成、使用する
  - Azure Policy を用いてデータ コネクタを構成する
...
- Microsoft Sentinel の Security Orchestration Automation and Response (SOAR) を構成する
  - Microsoft Sentinel プレイブックを作成する
...
- Microsoft Sentinel インシデントを管理する
  ...
  - Microsoft Sentinel のインシデントに対応する
