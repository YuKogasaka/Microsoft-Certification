...
- Enterprise Apps for Single Sign-On (SSO) の統合を計画、実装、監視する
  - 同意の設定を実装、構成する
  - Microsoft Defender for Cloud Apps/ADFS アプリケーション アクティビティ レポートを用いてアプリを見つける
  - アプリのアクセス管理を設計、実装する
  - アプリ管理のロールを設計、実装する
...
- Azure Active Directory を監視、保守する
  ...
  - Log Analytics/Microsoft Sentinel を用いて Azure AD 診断ログを有効化、統合する
  - サードパティ SIEM にサインインと監査ログをエクスポートする
  - KQL の使用を除く Log Analytics/Microsoft Sentinel を用いた Azure AD アクティビティをレビューする
