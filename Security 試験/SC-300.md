...
- Study Guide - Microsoft Identity and Access Administrator  
https://query.prod.cms.rt.microsoft.com/cms/api/am/binary/RE4Myp5

### 試験の対象者
この試験は、次の技術タスクを達成する能力を測定します : ID 管理ソリューションを実装する。認証およびアクセス管理ソリューションを実装する。アプリのアクセス管理を実装する。アイデンティティ ガバナンス戦略を計画、実装する。
...
この試験の受験者は、エンタープライズ アプリケーションへの安全な認証および承認アクセスを提供するなどのタスクを管理できる必要があります。また、ID およびアクセス環境のトラブルシューティング、監視、およびレポートの作成もできる必要があります。
(As an exam candidate, you should be able to manage tasks such as providing secure authentication and authorization access to enterprise applications. You should also be able to troubleshoot, monitor, and create reporting for the identity and access environment.)
...
(Design a strategy for monitoring Azure AD)
  - Azure AD コンソールを用いて、サインイン、監査、およびプロビジョニング ログを確認、分析する  
...
(Monitor Azure AD by using Log Analytics, including KQL queries)
  - ワークブックを用いて Azure AD を分析、Azure AD コンソールでレポートを作成する  
